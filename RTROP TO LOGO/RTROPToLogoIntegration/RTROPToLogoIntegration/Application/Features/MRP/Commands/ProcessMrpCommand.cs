...
namespace RTROPToLogoIntegration.Application.Features.MRP.Commands
{
    ...
    public class ProcessMrpCommandHandler
    {
        ...
        public async Task<bool> Handle(ProcessMrpCommand request)
        {
            ...
            var connectionString = _config.GetConnectionString("LogoConnection");

            LogoMRPModels mrpList;
            ...
            using (var connection = new SqlConnection(connectionString))
            {
                ...
                var ficheNo = await _stockRepository.GetLastMRPNumberAsync(request.FirmNo, request.PeriodNr, connection: connection);

                mrpList = new LogoMRPModels
                {
                    FICHENO = ficheNo,
                ...
                foreach (var item in request.Items)
                {
                    ...
                    var (itemRef, unitCode, _) = await _stockRepository.GetItemRefAndUnitByCodeAsync(item.ItemID, request.FirmNo, connection: connection);
                    ...
                    var stockQty = await _stockRepository.GetStockQuantityAsync(itemRef, request.FirmNo, request.PeriodNr, connection: connection);
                    var openPo = await _stockRepository.GetOpenPoQuantityAsync(itemRef, request.FirmNo, request.PeriodNr, connection: connection);
                    ...
                    if (netStock < item.ROP  && item.PlanningType == "MTS")
                    {
                        ...
                        string cardType = await _stockRepository.GetCardTypeAsync(item.ItemID, request.FirmNo, connection: connection);
                        ...
                        if (cardType == "10") // HAMMADDE
                        {
                            ...
                            clientRef = await _stockRepository.GetClientRefAsync(itemRef, request.FirmNo, connection: connection);
                        }
                        else if (cardType == "11") // YARI MAMUL
                        {
                            sourceIndex = ymWare;
                            meetType = 1; // Üretim
                            (bomRef, bomRevRef) = await _stockRepository.GetBomInfoAsync(itemRef, request.FirmNo, connection: connection);
                        }
                        ...
                        await _stockRepository.UpdateItemSpeCode2Async(itemRef, "MTS", request.FirmNo, connection: connection);
            ...
            // Logo Gönderimi (Connection Closed)
